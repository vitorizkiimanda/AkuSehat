<?php
  ...
  include 'db_connect.php';
    ...
    $query_login = mysqli_query($connect, "SELECT * FROM patients JOIN daily_health_data WHERE id_patient=id_pat && email_patient='$username' AND password_patient='$encrypt_password'");
    ...
    if(mysqli_num_rows($query_login)){
        ...
        if(mysqli_num_rows($query_login)){
            $row=mysqli_fetch_assoc($query_login);
            $data =array(
                'message' => "Login Success, but haven't daily health data ",
                'data' => $row,
                'status' => "200"
            );
        }
